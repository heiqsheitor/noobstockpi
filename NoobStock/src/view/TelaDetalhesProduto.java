...
public class TelaDetalhesProduto extends JPanel {
	...
	public TelaDetalhesProduto() {
		...
		setLayout(
				new MigLayout("fill, insets 25 35 25 35", "[grow, fill]20[grow, fill]", "[pref!][pref!][grow, fill]"));
		...
		JPanel linhaMiniCards = new JPanel(
				new MigLayout("insets 0, fillx", "[grow, fill]15[grow, fill]15[grow, fill]15[grow, fill]", "[fill]"));
		...
		listaMovimentacoes = new JPanel(new MigLayout("insets 0, fillx, wrap 1, gapy 10", "[grow, fill]", ""));
	...
	public void preencherDados(Produto p) {
		...
		lblValorLocal.setText(
				(p.getLocalização() != null && !p.getLocalização().isEmpty()) ? p.getLocalização() : "Não alocado");

		lblValorFornecedor.setText(
				(p.getFornecedor() != null && !p.getFornecedor().isEmpty()) ? p.getFornecedor() : "Sem fornecedor");

		lblValorCategoria.setText(
				(p.getCategoria() != null && !p.getCategoria().isEmpty()) ? p.getCategoria() : "Não categorizado");
		...
		lblValorAtualizacao.setText((dataAtu != null && !dataAtu.isEmpty()) ? dataAtu : "Não informado");
	}
	...
	private JPanel criarItemHistorico(HistoricoMovimentacao h) {
		JPanel item = new JPanel(new MigLayout("insets 6 0 6 0, fillx", "[pref!][grow][right]", "[]"));
		...
		String descricao = h.getObservacao().isEmpty() ? "Saída de estoque" : h.getObservacao();
